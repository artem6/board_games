...
const rules: { [key: string]: { match: string; replacement: string; type: number }[] } = {
  a: [
...
export const stemmer = (value: string) => applyRules(String(value).toLowerCase(), true);

const applyRules = (value: string, isintact: boolean): string => {
  const ruleset = rules[value.charAt(value.length - 1)];
  if (!ruleset) return value;

  for (let i = 0; i < ruleset.length; i++) {
    const rule = ruleset[i];
    if (!isintact && rule.type === intact) continue;
    const breakpoint = value.length - rule.match.length;
    if (breakpoint < 0 || value.slice(breakpoint) !== rule.match) continue;
    if (rule.type === protect) return value;
    const next = value.slice(0, breakpoint) + rule.replacement;
    if (!acceptable(next)) continue;
    if (rule.type === cont) return applyRules(next, false);
    return next;
  }
...
const acceptable = (value: string) =>
  vowels.test(value.charAt(0)) ? value.length > 1 : value.length > 2 && vowels.test(value);
